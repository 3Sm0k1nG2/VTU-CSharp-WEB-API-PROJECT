...
{
    public class ServiceRepository : RepositoryBase<ServiceEntity, ServiceDto, ServiceDto>
    {
        public ServiceRepository(WorkOrderDbContext context, IMemoryCache memoryCache) : base(context, memoryCache) { }
        ...
        )
        {
            ...
            var data = _memoryCache.Get<IEnumerable<object>>(Cache.Keys.Services);

            if (data is null)
            {
                data = this._mapper.Map(query.ToList());
                _memoryCache.Set(Cache.Keys.Services, data, Cache.Options.TimeSpans.Day);
            }
